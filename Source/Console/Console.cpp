...
int Tag(const TCHAR * pFilename, const TCHAR * pTagString)
{
    ...
    try
    {
        spAPEDecompress.Assign(CreateIAPEDecompress(pFilename, &nFunctionRetVal));
        ...
        for (int nCount = 0; true; nCount++)
        {
            ...
            if (pEqual != NULL)
            {
                int nCharacters = pEqual - pParameter;
                TCHAR* pLeft = new TCHAR[nCharacters + 1];
                _tcsncpy(pLeft, pParameter, nCharacters);
                pLeft[nCharacters] = 0;

                nCharacters = _tcslen(&pEqual[1]);
                TCHAR* pRight = new TCHAR[nCharacters + 1];
                _tcscpy(pRight, &pEqual[1]);
